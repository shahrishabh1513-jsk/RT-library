...
function loadUserData() {
    ...
    if (storedUser) {
        ...
        document.getElementById('welcomeName').textContent = currentUser.name.split(' ')[1] || currentUser.name;
        ...
        facultyBorrowedBooks = facultyTransactions.filter(t => t.status === 'active');
    } else {
...
function loadDashboardData() {
    document.getElementById('borrowedCount').textContent = facultyBorrowedBooks.length;
    
    const overdue = facultyBorrowedBooks.filter(book => new Date(book.dueDate) < new Date()).length;
    document.getElementById('overdueCount').textContent = overdue;
    
    // Load recommended books (research-oriented)
...
function loadBorrowedBooks() {
    ...
    tbody.innerHTML = facultyBorrowedBooks.map(transaction => {
        const book = getBookById(transaction.bookId);
        const dueDate = new Date(transaction.dueDate);
        const isOverdue = dueDate < new Date();
        const daysLeft = Math.ceil((dueDate - new Date()) / (1000 * 60 * 60 * 24));
        let status = isOverdue ? 'Overdue' : `Due in ${daysLeft} days (90-day period)`;
    ...
    grid.innerHTML = availableBooks.map(book => `
        <div class="book-card" onclick="showBookDetails(${book.id})">
            ...
            <div class="book-details">
                ...
                <p style="font-size:0.7rem;color:var(--success);margin-top:0.3rem"><i class="fas fa-clock"></i> 90-day borrowing period</p>
            </div>
        </div>
    `).join('');
}
...
function loadProfile() {
    ...
    card.innerHTML = `
        ...
        <div class="info-row">
            <div class="info-label"><i class="fas fa-id-card"></i> Faculty ID</div>
        ...
        <div class="info-row">
            ...
            <div class="info-value">${currentUser.department || 'Computer Science'}</div>
        </div>
        <div class="info-row">
            <div class="info-label"><i class="fas fa-user-tie"></i> Position</div>
            <div class="info-value">${currentUser.position || 'Professor'}</div>
        </div>
        <div class="info-row">
            <div class="info-label"><i class="fas fa-flask"></i> Research Credits</div>
            <div class="info-value">150 credits</div>
        </div>
        <div class="info-row">
        ...
        <div class="info-row">
            ...
            <div class="info-value">${facultyTransactions.length}</div>
        </div>
        <div class="info-row">
            <div class="info-label"><i class="fas fa-clock"></i> Borrowing Period</div>
            <div class="info-value">90 Days (Extended Faculty Access)</div>
        </div>
    `;
}
...
function loadHistory() {
    ...
    const completed = facultyTransactions.filter(t => t.status === 'returned');
...
function showBookDetails(bookId) {
    ...
    const isAlreadyBorrowed = facultyBorrowedBooks.some(t => t.bookId === bookId);
    ...
    document.getElementById('modalContent').innerHTML = `
        ...
        <div style="background:var(--light);padding:1rem;border-radius:12px;margin-bottom:1rem">
            ...
            <p><strong><i class="fas fa-clock"></i> Faculty Borrowing:</strong> 90-day period</p>
        </div>
...
function borrowBookAPI(userId, bookId) {
    ...
    // Faculty gets 90 days borrowing period
    const transaction = {
        id: Date.now(),
        userId: userId,
        bookId: bookId,
